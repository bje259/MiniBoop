...
function main(state) {
    ...
    try {
        ...
        const nestedCheckResults = initInput.replaceAll(nestedRegex, (m, ...args) => {
            ...
            if (loopvals && loopvar) {
                const varParts = loopvar.split(/(?<!\\);/).map(ele => ele.trim().replace(/\\;/g, ";"));
                const vals = loopvals.split(",").map(ele => ele.trim());
                let combinedResult = "";
                for (const val of vals) {
                    const valParts = val.split(/(?<!\\);/).map(ele => ele.trim().replace(/\\;/g, ";"));
                    if (valParts.length !== varParts.length) {
                        state.postError(`Loop variable count (${varParts.length}) does not match value count (${valParts.length}).`);
                        continue;
                    }
